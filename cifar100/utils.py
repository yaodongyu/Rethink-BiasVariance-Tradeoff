...
def get_subsample_dataset_label_noise(trainset, train_size, noise_size):
    subset = torch.randperm(len(trainset))[:train_size]
    trainsubset = copy.deepcopy(trainset)
    trainsubset.data = [trainsubset.data[idx] for idx in subset.numpy()]
    trainsubset.targets = [trainsubset.targets[idx] for idx in subset.numpy()]
    ######## shuffle
    ...
    for idx in range(train_size - noise_size, train_size):
        trainsubset.targets[idx] = shuffle_targets_subset[idx - train_size]
